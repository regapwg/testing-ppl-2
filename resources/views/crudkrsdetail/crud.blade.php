...
<div class="nk-fmg-body-head d-none d-lg-flex">
    <div class="nk-fmg-search">
        <a href="{{ route('krs.list') }}"><button type="button" class="btn btn-info"><em class="icon fas fa-arrow-left"></em> <span>KRS</span></button></a>  &nbsp;&nbsp;
        <h4 class="card-title text-primary">
            <i class='{{$icon}}' data-toggle='tooltip' data-placement='bottom' title='Data {{$subtitle}}'></i>  
            {{ strtoupper("Data ".$subtitle.": \" KRS ".$krs->nama_krs." - ".$nim." \"") }}
        </h4>
    </div>
    <div class="nk-fmg-actions">
        <div class="btn-group"> 
            <a href="/admin/{{ $krs_id }}/create/krsdetail" class="btn btn-sm btn-primary" onclick="buttondisable(this)"><em class="icon fas fa-plus"></em> <span>Add KRS Detail</span></a>
        </div>
    </div>
</div>
...
$(document).ready(function() {
    table = $('#{{$table_id}}').DataTable({
        ...
        ajax: {
            url: '{{ route("krsdetail.listData", $krs_id) }}',
            type:"POST",
